...
public class CasualEventValidator : AbstractValidator<CasualEvent>
{
    public CasualEventValidator()
    {
        ...
        RuleFor(x => x.MaximumCapacity).Cascade(CascadeMode.Stop).NotNull().WithMessage("Maximum Capacity is required");
        RuleFor(x => x.Location).NotEmpty().WithMessage("Location is required");
        RuleFor(x => x.StartDate).Cascade(CascadeMode.Stop).NotNull().WithMessage("Start Date is required");
        RuleFor(x => x.EndDate).NotNull().WithMessage("End Date is required");
        RuleFor(x => x.Description).NotEmpty().WithMessage("Description is required");
        RuleFor(x => x.MaximumCapacity).GreaterThan(x => x.CurrentCapacity).WithMessage("Current Capacity cannot be greater than Maximum Capacity");
